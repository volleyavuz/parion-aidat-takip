...
public class MainActivityV33 extends MainActivityV31 {
    ...
    @Override void showProfile(long id){super.showProfile(id);relabelReportButton(root);}
    ...
    @Override void shareReport(long id){
        ...
        new AlertDialog.Builder(this).setTitle("BİLANÇO ZAMAN ARALIĞI").setItems(opts,(d,w)->{
            int months=w==1?12:w==2?6:w==3?3:0;createRollingReport(id,months,opts[w]);
    ...
    void createRollingReport(long id,int monthCount,String filterName){
        ...
        HashMap<Integer,PayRec> pays=new HashMap<>();Cursor p=db.payments(id);while(p.moveToNext()){
            int yr=p.getInt(p.getColumnIndexOrThrow("year")),mo=p.getInt(p.getColumnIndexOrThrow("month"));
            pays.put(yr*100+mo,new PayRec(s(p,"marker"),p.getInt(p.getColumnIndexOrThrow("amount"))));
        }p.close();

        Calendar now=Calendar.getInstance();int currentKey=now.get(Calendar.YEAR)*100+(now.get(Calendar.MONTH)+1);int nextKey=shiftMonth(currentKey,1);
        int startKey;
        ...
        ArrayList<Integer> months=new ArrayList<>();for(int k=startKey;;k=shiftMonth(k,1)){months.add(k);if(k==nextKey)break;if(months.size()>241)break;}
        ...
        c.drawText("Dönem: "+keyLabel(startKey)+" – "+keyLabel(nextKey),60,268,q);
        ...
        for(int key:months){
            int yr=key/100,mo=key%100;PayRec r=pays.get(key);if(r==null)r=new PayRec("",0);int expected=expectedFeeAt(id,yr,mo,r);boolean active=activeAt(yr,mo,start,end,restart);String status;int color;String date="—";boolean futureWaiting=(key==nextKey);
            if(futureWaiting&&r.amount<=0&&!isDate(r.marker)&&!"X".equals(r.marker)){
                if(!active){status="AKTİF DEĞİL";color=Color.rgb(225,225,225);expected=0;}
                else if("BURSLU".equalsIgnoreCase(sibling)||(expected==0&&currentMonthlyFee(id)==0)){status="BURSLU";color=Color.rgb(225,225,225);expected=0;}
                else {status="BEKLİYOR";color=Color.WHITE;}
            ...
            else if(key==currentKey){status="ÖDEME DÖNEMİ";color=YELLOW;}
            else {status="ÖDEME GECİKTİ";color=RED;}

            if(!futureWaiting&&expected>0&&active&&!"X".equals(r.marker))totalExpected+=expected;if(r.amount>0)totalPaid+=r.amount;
        ...
        q.setColor(Color.DKGRAY);q.setTypeface(Typeface.DEFAULT);q.setTextSize(18);c.drawText("Not: Kayıt tarihinden önceki aylar gösterilmez. Gelecekte yalnızca bir sonraki ay BEKLİYOR olarak gösterilir ve toplama borç olarak eklenmez.",60,y+90,q);
