...
public class MainActivityV417 extends MainActivityV416 {
    ...
    private void uploadMedia417(long id,Uri uri,boolean isForm,boolean missing){
        ...
        media417.execute(()->{try{
            ...
            UploadResult417 up=put417(bucket,path,jpg);if(up.code<200||up.code>=300)throw new IOException("STORAGE "+up.code+cleanError417(up.body));
        ...
        }catch(Exception e){String m=e.getMessage();runOnUiThread(()->{showError417((isForm?"KAYIT FORMU":"FOTOĞRAF")+" YÜKLENEMEDİ",m==null?"BAĞLANTI":m);returnAfter417(id,isForm,missing);});}});
    }
