...
public class MainActivityV750 extends MainActivityV749 {
    ...
    protected void onRemoteApplied750(boolean athleteChanged,boolean attendanceChanged,boolean materialChanged,boolean membershipChanged){
        if((athleteChanged||attendanceChanged||materialChanged||membershipChanged)&&"HOME".equals(page))runOnUiThread(this::showHome);
    }
    ...
    private void deltaPass750(boolean announce){
        ...
        try{
            ...
            if(announce){final int p=pushed,a=athleteDelta;runOnUiThread(()->toast("EŞZAMANLAMA TAMAM • "+p+" SPORCU GÖNDERİLDİ • "+a+" SPORCU DELTA • TÜM VERİ TÜRLERİ EŞİTLENDİ"));}
            onRemoteApplied750(athleteDelta>0,attendanceChanged,materialChanged,membershipChanged);
    ...
    private void pullMaterial750()throws Exception{
        if(pendingKindCount750("MATERIAL")>0)return;JSONArray p=new JSONArray(getAuthed750("/rest/v1/material_products?select=id,name,current_price,active&order=name.asc").body),t=new JSONArray(getAuthed750("/rest/v1/material_transactions?select=id,legacy_local_id,athlete_id,product,qty,unit_price,total,paid_amount,issued_date,payment_date,note&order=created_at.asc").body);SQLiteDatabase d=db.getWritableDatabase();d.beginTransaction();try{invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,true);d.delete("material_products",null,null);for(int i=0;i<p.length();i++){JSONObject o=p.getJSONObject(i);ContentValues v=new ContentValues();v.put("name",o.optString("name",""));v.put("currentPrice",o.optInt("current_price",0));v.put("active",o.optBoolean("active",true)?1:0);d.insertWithOnConflict("material_products",null,v,SQLiteDatabase.CONFLICT_REPLACE);}d.delete("material_transactions",null,null);for(int i=0;i<t.length();i++){JSONObject o=t.getJSONObject(i);ContentValues v=new ContentValues();v.put("cloudId",o.optString("id",""));v.put("athleteId",o.optLong("athlete_id",-1));v.put("product",o.optString("product",""));v.put("qty",o.optInt("qty",0));v.put("unitPrice",o.optInt("unit_price",0));v.put("total",o.optInt("total",0));v.put("paidAmount",o.optInt("paid_amount",0));v.put("issuedDate",o.optString("issued_date","") );v.put("paymentDate",o.isNull("payment_date")?"":o.optString("payment_date",""));v.put("note",o.optString("note",""));long local=o.optLong("legacy_local_id",0);if(local>0){v.put("id",local);d.insertWithOnConflict("material_transactions",null,v,SQLiteDatabase.CONFLICT_REPLACE);}else d.insert("material_transactions",null,v);}invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,false);d.setTransactionSuccessful();}finally{try{invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,false);}catch(Exception ignored){}d.endTransaction();}
    }

    private void pushMembership750()throws Exception{
        long stamp=pendingStamp750("MEMBERSHIP");JSONArray a=new JSONArray();Cursor c=db.getReadableDatabase().rawQuery("SELECT athleteId,endDate FROM athlete_restart_periods",null);while(c.moveToNext())a.put(new JSONObject().put("athlete_id",c.getLong(0)).put("end_date",c.getString(1)));c.close();postRpc750("parion_sync_restart_ends_v1",new JSONObject().put("p_rows",a));db.getWritableDatabase().delete("pending_sync","kind='MEMBERSHIP' AND created_at<=?",new String[]{String.valueOf(stamp)});
    }
    private void pullMembership750()throws Exception{
        if(pendingKindCount750("MEMBERSHIP")>0)return;JSONArray a=new JSONArray(getAuthed750("/rest/v1/athlete_membership_events?select=legacy_id,event_type,effective_date&event_type=eq.restart_end&order=effective_date.asc").body);SQLiteDatabase d=db.getWritableDatabase();d.beginTransaction();try{invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,true);d.delete("athlete_restart_periods",null,null);for(int i=0;i<a.length();i++){JSONObject o=a.getJSONObject(i);ContentValues v=new ContentValues();v.put("athleteId",o.optLong("legacy_id",-1));v.put("endDate",o.optString("effective_date",""));d.insert("athlete_restart_periods",null,v);}invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,false);d.setTransactionSuccessful();}finally{try{invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,false);}catch(Exception ignored){}d.endTransaction();}
    }

    private JSONObject fetchDomains750()throws Exception{return postRpc750("parion_sync_domain_revisions_v1",new JSONObject());}
    private boolean domainChanged750(JSONObject d,String table){long now=d.optLong(table,0),old=cloudPrefs.getLong("domain_rev_750_"+table,-1);return old<0||now!=old;}
    private void saveDomains750(JSONObject d){android.content.SharedPreferences.Editor e=cloudPrefs.edit();for(String t:DOMAIN_TABLES)e.putLong("domain_rev_750_"+t,d.optLong(t,0));e.apply();}
    private long pendingStamp750(String kind){try{Cursor c=db.getReadableDatabase().rawQuery("SELECT MAX(created_at) FROM pending_sync WHERE kind=?",new String[]{kind});long x=0;if(c.moveToFirst()&&!c.isNull(0))x=c.getLong(0);c.close();return x;}catch(Exception e){return 0;}}
    ...
    private HttpResult getAuthed750(String path)throws Exception{String token=cloudPrefs.getString("access_token","");HttpResult r=request("GET",SUPABASE_URL+path,null,token);if(r.code==401&&refreshSession()){token=cloudPrefs.getString("access_token","");r=request("GET",SUPABASE_URL+path,null,token);}if(r.code<200||r.code>=300)throw new Exception("GET "+path+" HTTP "+r.code);return r;}
    private JSONObject fetchDelta750(String since)throws Exception{JSONObject b=new JSONObject();if(since!=null&&!since.isEmpty())b.put("p_since",since);return postRpc750("parion_delta_snapshot_v1",b);}
