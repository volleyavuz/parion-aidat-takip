...
/** v4.2.35 - full entity delta sync while preserving the stable V750 visual shell. */
public class MainActivityV750 extends MainActivityV749 {
    ...
    @Override void base(String title, boolean back){
        ...
        final int gen=++financeGeneration750;final View builtRoot=root;builtRoot.setAlpha(0f);
        builtRoot.post(()->builtRoot.post(()->builtRoot.postOnAnimation(()->{if(gen==financeGeneration750&&root==builtRoot)builtRoot.setAlpha(1f);})));}
    ...
    private void setupExtraSync750(){
        ...
        if(!cloudPrefs.getBoolean("membership_seeded_750",false)){
            Cursor c=d.rawQuery("SELECT athleteId FROM athlete_restart_periods",null);while(c.moveToNext())enqueueKind750("MEMBERSHIP",String.valueOf(c.getLong(0)));c.close();cloudPrefs.edit().putBoolean("membership_seeded_750",true).apply();
        }
    }
    ...
    private void deltaPass750(boolean announce){
        ...
        try{
            ...
            if(attPending)syncAttendance750();
            if(matPending)pushMaterial750();
            if(memPending)pushMembership750();
            ...
            int athleteDelta=athleteChanged?syncAthleteDelta750():0;
            if(attendanceChanged&&!attPending)syncAttendance750();
            if(materialChanged)pullMaterial750();
            if(membershipChanged)pullMembership750();
            saveDomains750(domains);

            if(announce){final int p=pushed,a=athleteDelta;runOnUiThread(()->{toast("EŞZAMANLAMA TAMAM • "+p+" SPORCU GÖNDERİLDİ • "+a+" SPORCU DELTA • YOKLAMA/MALZEME/DURUM EŞİTLENDİ");showHome();});}
            else if(athleteDelta>0||attendanceChanged||materialChanged||membershipChanged)runOnUiThread(this::showHome);
        }catch(Exception e){if(announce){String m=rootCause750(e);runOnUiThread(()->toast("EŞZAMANLAMA DURDU • YEREL VERİ KORUNDU • "+m));}}
        finally{running750.set(false);}
    }

    private int syncAthleteDelta750()throws Exception{
        String since=cloudPrefs.getString("delta_cursor_750","");JSONObject s=fetchDelta750(since);
        JSONArray aa=arr750(s,"athletes"),pp=arr750(s,"payments"),ff=arr750(s,"fees"),dd=arr750(s,"deleted"),ids=arr750(s,"changed_ids");String cursor=s.optString("cursor_at","");if(cursor.isEmpty())throw new Exception("DELTA CURSOR YOK");
    ...
    private void pushMembership750()throws Exception{
        ArrayList<String> ids=new ArrayList<>();ArrayList<Long> stamps=new ArrayList<>();JSONArray e=new JSONArray();Cursor c=db.getReadableDatabase().rawQuery("SELECT entity_key,created_at FROM pending_sync WHERE kind='MEMBERSHIP' ORDER BY created_at",null);while(c.moveToNext()){String id=c.getString(0);ids.add(id);stamps.add(c.getLong(1));Cursor z=db.getReadableDatabase().rawQuery("SELECT restartEndDate FROM athlete_restart_periods WHERE athleteId=?",new String[]{id});String dt="";if(z.moveToFirst())dt=nz750(z.getString(0));z.close();e.put(new JSONObject().put("legacy_id",Long.parseLong(id)).put("event_date",dt));}c.close();if(e.length()==0)return;postRpc750("parion_sync_restart_ends_v1",new JSONObject().put("p_events",e));SQLiteDatabase d=db.getWritableDatabase();for(int i=0;i<ids.size();i++)d.delete("pending_sync","kind='MEMBERSHIP' AND entity_key=? AND created_at<=?",new String[]{ids.get(i),String.valueOf(stamps.get(i))});
    }

    private void pullMembership750()throws Exception{
        if(pendingKindCount750("MEMBERSHIP")>0)return;JSONArray a=new JSONArray(getAuthed750("/rest/v1/athlete_membership_events?event_type=eq.RESTART_END&select=legacy_id,event_date,updated_at&order=legacy_id.asc").body);SQLiteDatabase d=db.getWritableDatabase();d.beginTransaction();try{if(pendingKindCount750("MEMBERSHIP")>0)return;invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,true);d.delete("athlete_restart_periods",null,null);for(int i=0;i<a.length();i++){JSONObject o=a.getJSONObject(i);ContentValues v=new ContentValues();v.put("athleteId",o.optLong("legacy_id",-1));v.put("restartEndDate",o.optString("event_date",""));d.insertWithOnConflict("athlete_restart_periods",null,v,SQLiteDatabase.CONFLICT_REPLACE);}invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,false);d.setTransactionSuccessful();}finally{try{invoke740("guard740",new Class<?>[]{SQLiteDatabase.class,boolean.class},d,false);}catch(Exception ignored){}d.endTransaction();}
    }

    private long pendingStamp750(String kind){Cursor c=db.getReadableDatabase().rawQuery("SELECT COALESCE(MAX(created_at),0) FROM pending_sync WHERE kind=?",new String[]{kind});long x=0;if(c.moveToFirst())x=c.getLong(0);c.close();return x;}
    private JSONObject fetchDomains750()throws Exception{JSONArray a=new JSONArray(getAuthed750("/rest/v1/parion_sync_domain_state?select=source_table,revision,changed_at").body);JSONObject x=new JSONObject();for(int i=0;i<a.length();i++){JSONObject o=a.getJSONObject(i);x.put(o.optString("source_table",""),o.optLong("revision",0));}return x;}
    private boolean domainChanged750(JSONObject x,String table){long now=x.optLong(table,0),old=cloudPrefs.getLong("domain_rev_750_"+table,0);return now!=old;}
    ...
    private HttpResult getAuthed750(String path)throws Exception{String token=cloudPrefs.getString("access_token","");HttpResult r=request("GET",SUPABASE_URL+path,null,token);if(r.code==401&&refreshSession()){token=cloudPrefs.getString("access_token","");r=request("GET",SUPABASE_URL+path,null,token);}if(r.code<200||r.code>=300)throw new Exception("BULUT HTTP "+r.code+" • "+path);return r;}
    private JSONObject postRpc750(String name,JSONObject body)throws Exception{String token=cloudPrefs.getString("access_token","");HttpResult r=request("POST",SUPABASE_URL+"/rest/v1/rpc/"+name,body.toString(),token);if(r.code==401&&refreshSession()){token=cloudPrefs.getString("access_token","");r=request("POST",SUPABASE_URL+"/rest/v1/rpc/"+name,body.toString(),token);}if(r.code<200||r.code>=300)throw new Exception(name+" HTTP "+r.code);return new JSONObject(r.body);}
    private JSONObject fetchDelta750(String since)throws Exception{String body=(since==null||since.isEmpty())?"{\"p_since\":null}":new JSONObject().put("p_since",since).toString();return postRpc750("parion_delta_snapshot_v1",new JSONObject(body));}
