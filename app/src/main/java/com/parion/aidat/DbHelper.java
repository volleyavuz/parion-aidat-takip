...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private void createSyncTables(SQLiteDatabase d){
        d.execSQL("CREATE TABLE IF NOT EXISTS sync_state(entity TEXT NOT NULL,entityKey TEXT NOT NULL,cloudUpdatedAt TEXT,localHash TEXT,lastSyncedAt INTEGER NOT NULL DEFAULT 0,PRIMARY KEY(entity,entityKey))");
    }

    private boolean hasColumn(SQLiteDatabase d,String table,String col){
        Cursor c=null;try{c=d.rawQuery("PRAGMA table_info("+table+")",null);while(c.moveToNext())if(col.equalsIgnoreCase(c.getString(c.getColumnIndexOrThrow("name"))))return true;}catch(Exception ignored){}finally{if(c!=null)c.close();}return false;
    }
    ...
    public Cursor athletes(String q,String status){
        String sql="SELECT * FROM athletes WHERE TRIM(COALESCE(deletedAt,''))=''";ArrayList<String> args=new ArrayList<>();
    ...
    public int count(String status){
        Cursor c=getReadableDatabase().rawQuery(status==null?"SELECT COUNT(*) FROM athletes WHERE TRIM(COALESCE(deletedAt,''))=''":"SELECT COUNT(*) FROM athletes WHERE TRIM(COALESCE(deletedAt,''))='' AND status=?",status==null?null:new String[]{status});
        c.moveToFirst();int x=c.getInt(0);c.close();return x;
    }
