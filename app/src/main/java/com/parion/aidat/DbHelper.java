...
/**
 * v4 cloud-first local cache.
 * The APK no longer embeds athlete/payment seed data. A fresh install starts
 * with an empty cache and receives the current dataset from Supabase.
 */
public class DbHelper extends SQLiteOpenHelper {
    ...
    @Override public void onCreate(SQLiteDatabase d){
        ...
        createFeeHistory(d);
        createMaterialTables(d);
        createRegistrationForms(d);
    }

    private void createFeeHistory(SQLiteDatabase d){
        d.execSQL("CREATE TABLE IF NOT EXISTS fee_history(id INTEGER PRIMARY KEY AUTOINCREMENT,athleteId INTEGER,year INTEGER,effectiveMonth INTEGER,fee INTEGER,UNIQUE(athleteId,year,effectiveMonth))");
    }

    private void createMaterialTables(SQLiteDatabase d){
    ...
    private void createRegistrationForms(SQLiteDatabase d){
        d.execSQL("CREATE TABLE IF NOT EXISTS registration_forms(athleteId INTEGER PRIMARY KEY,fileRef TEXT NOT NULL DEFAULT '',mimeType TEXT NOT NULL DEFAULT '',originalName TEXT NOT NULL DEFAULT '',updatedAt TEXT NOT NULL DEFAULT '')");
    }

    @Override public void onUpgrade(SQLiteDatabase d,int oldVersion,int newVersion){
        if(oldVersion<2)createFeeHistory(d);
        if(oldVersion<4){
            addColumn(d,"athletes","birthDate","TEXT");
            addColumn(d,"athletes","tckn","TEXT");
            addColumn(d,"athletes","summerCall","INTEGER NOT NULL DEFAULT 0");
            addColumn(d,"athletes","winterCall","INTEGER NOT NULL DEFAULT 0");
            addColumn(d,"athletes","deletedAt","TEXT");
            addColumn(d,"athletes","deletedPrevStatus","TEXT");
            createMaterialTables(d);
            createRegistrationForms(d);
        }
    }

    private void addColumn(SQLiteDatabase d,String table,String col,String type){
        Cursor c=null;boolean exists=false;
        try{c=d.rawQuery("PRAGMA table_info("+table+")",null);while(c.moveToNext())if(col.equalsIgnoreCase(c.getString(c.getColumnIndexOrThrow("name")))){exists=true;break;}}finally{if(c!=null)c.close();}
        if(!exists)d.execSQL("ALTER TABLE "+table+" ADD COLUMN "+col+" "+type);
    }

    @Override public SQLiteDatabase getWritableDatabase(){return super.getWritableDatabase();}

    public Cursor athletes(String q,String status){
        String sql="SELECT * FROM athletes WHERE 1=1";ArrayList<String> args=new ArrayList<>();
        if(q!=null&&!q.trim().isEmpty()){sql+=" AND name LIKE ?";args.add("%"+q.trim().toUpperCase(Locale.forLanguageTag("tr-TR"))+"%");}
        if(status!=null&&!status.equals("TÜMÜ")){sql+=" AND status=?";args.add(status);}
        sql+=" ORDER BY CASE status WHEN 'AKTİF' THEN 0 WHEN 'ARA VERDİ' THEN 1 ELSE 2 END,name COLLATE NOCASE";
        return getReadableDatabase().rawQuery(sql,args.toArray(new String[0]));
    }
    ...
    public void updatePayment(long athleteId,int month,String marker,int amount){
        ContentValues v=new ContentValues();v.put("athleteId",athleteId);v.put("year",2026);v.put("month",month);v.put("marker",marker);v.put("amount",amount);
        getWritableDatabase().insertWithOnConflict("payments",null,v,SQLiteDatabase.CONFLICT_REPLACE);
    }
    ...
    public void setFeeFromMonth(long athleteId,int month,int fee){
        ContentValues v=new ContentValues();v.put("athleteId",athleteId);v.put("year",2026);v.put("effectiveMonth",month);v.put("fee",fee);
        getWritableDatabase().insertWithOnConflict("fee_history",null,v,SQLiteDatabase.CONFLICT_REPLACE);
        Calendar now=Calendar.getInstance();if(now.get(Calendar.YEAR)>2026 || (now.get(Calendar.YEAR)==2026 && month<=now.get(Calendar.MONTH)+1)){ContentValues a=new ContentValues();a.put("monthlyFee",fee);getWritableDatabase().update("athletes",a,"id=?",new String[]{String.valueOf(athleteId)});}
    }
