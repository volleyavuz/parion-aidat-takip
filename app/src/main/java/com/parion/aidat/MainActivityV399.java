...
public class MainActivityV399 extends MainActivityV398 {
    ...
    private void ensureRegistrationForms399(){
        try{db.getWritableDatabase().execSQL("CREATE TABLE IF NOT EXISTS registration_forms (athleteId INTEGER PRIMARY KEY, fileRef TEXT NOT NULL DEFAULT '', mimeType TEXT NOT NULL DEFAULT '', originalName TEXT NOT NULL DEFAULT '', updatedAt TEXT NOT NULL DEFAULT '')");}catch(Exception ignored){}
    }

    @Override void showHome(){super.showHome();addMissingFormsCard399();}
    ...
    private boolean hasForm399(long id){
        Cursor c=db.getReadableDatabase().rawQuery("SELECT fileRef FROM registration_forms WHERE athleteId=?",new String[]{String.valueOf(id)});boolean ok=false;
        if(c.moveToFirst()){String r=c.getString(0);ok=r!=null&&!r.trim().isEmpty()&&formFileExists399(r);}c.close();return ok;
    }
    ...
    private void saveRegistrationForm399(Uri uri,long id){
        try{
            ...
            db.getWritableDatabase().execSQL("INSERT OR REPLACE INTO registration_forms(athleteId,fileRef,mimeType,originalName,updatedAt) VALUES(?,?,?,?,datetime('now'))",new Object[]{id,"USER:"+fn,mime,fn});toast("Kayıt formu kaydedildi.");
        }catch(Exception e){toast("Kayıt formu eklenemedi: "+e.getMessage());}
    }

    private void showRegistrationForm399(long id){
        Cursor c=db.getReadableDatabase().rawQuery("SELECT fileRef,mimeType FROM registration_forms WHERE athleteId=?",new String[]{String.valueOf(id)});if(!c.moveToFirst()){c.close();toast("Kayıt formu bulunamadı.");return;}String ref=c.getString(0),mime=c.getString(1);c.close();
    ...
    private void showImageForm399(String ref)throws Exception{
        Bitmap bm;try(InputStream in=openFormStream399(ref)){bm=BitmapFactory.decodeStream(in);}if(bm==null)throw new IOException("Görsel okunamadı");
    ...
    private void showPdf399(String ref)throws Exception{
        ...
        int max=Math.min(renderer.getPageCount(),20);for(int i=0;i<max;i++){PdfRenderer.Page p=renderer.openPage(i);int w=1000,h=Math.max(1,(int)(w*((float)p.getHeight()/p.getWidth())));Bitmap b=Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);b.eraseColor(Color.WHITE);p.render(b,null,null,PdfRenderer.Page.RENDER_MODE_FOR_DISPLAY);p.close();ImageView iv=new ImageView(this);iv.setImageBitmap(b);iv.setAdjustViewBounds(true);pages.addView(iv,new LinearLayout.LayoutParams(-1,-2));}
