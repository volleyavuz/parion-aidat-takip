...
/**
 * v4.2.19 - loop-safe realtime via a non-sensitive sync signal table.
 *
 * Realtime listens ONLY to public.parion_sync_signal. Domain tables stay behind
 * the existing REST/RPC/RLS model. A signal event means "something changed";
 * the app then performs one debounced catch-up through the safe canonical pull.
 */
            ...
            socket741=wsClient741.newWebSocket(req,new WebSocketListener(){
                ...
                @Override public void onMessage(WebSocket ws,String text){
                    try{
                        ...
                        if("postgres_changes".equals(x.optString("event"))){
                            JSONObject p=x.optJSONObject("payload");
                            JSONObject d=p==null?null:p.optJSONObject("data");
                            if(d==null||"parion_sync_signal".equals(d.optString("table","")))queueCatchup741();
                        }
