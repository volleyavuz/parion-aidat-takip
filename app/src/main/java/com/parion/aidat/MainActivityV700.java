...
/** v4.1.15 HOME callback aggregate + single setContentView + root stage profiler. */
public class MainActivityV700 extends MainActivityV699 {
    ...
    static volatile long measureTotal700=0L,layoutTotal700=0L,drawTotal700=0L;
    static volatile long measureMax700=0L,layoutMax700=0L,drawMax700=0L;
    static volatile int measureCount700=0,layoutCount700=0,drawCount700=0;

    static synchronized void resetCallbackStats700(){
        maxCallbackCost700=0L;maxCallbackRequested700=0L;maxCallbackSeq700=0;
        totalCallbackCost700=0L;totalCallbackQueue700=0L;callbackRunCount700=0;
        measureTotal700=layoutTotal700=drawTotal700=0L;
        measureMax700=layoutMax700=drawMax700=0L;
        measureCount700=layoutCount700=drawCount700=0;
    }

    static synchronized void recordCallback700(long cost,long queue,long requested,int seq){
        callbackRunCount700++;totalCallbackCost700+=cost;totalCallbackQueue700+=Math.max(0L,queue);
        if(cost>maxCallbackCost700){maxCallbackCost700=cost;maxCallbackRequested700=requested;maxCallbackSeq700=seq;}
    }

    static synchronized void recordMeasure700(long ms){measureCount700++;measureTotal700+=ms;if(ms>measureMax700)measureMax700=ms;}
    static synchronized void recordLayout700(long ms){layoutCount700++;layoutTotal700+=ms;if(ms>layoutMax700)layoutMax700=ms;}
    static synchronized void recordDraw700(long ms){drawCount700++;drawTotal700+=ms;if(ms>drawMax700)drawMax700=ms;}

    @Override void base(String title, boolean back) {
        if (page == null || !"HOME".equalsIgnoreCase(page)) {super.base(title, back);return;}
        FastHomeRoot700 fast = new FastHomeRoot700(this);fast.setOrientation(LinearLayout.VERTICAL);fast.setBackgroundColor(BG);root = fast;setContentView(root);
    ...
    static final class FastHomeRoot700 extends LinearLayout {
        ...
        @Override protected void onMeasure(int w,int h){long s=SystemClock.elapsedRealtime();super.onMeasure(w,h);recordMeasure700(SystemClock.elapsedRealtime()-s);}
        @Override protected void onLayout(boolean c,int l,int t,int r,int b){long s=SystemClock.elapsedRealtime();super.onLayout(c,l,t,r,b);recordLayout700(SystemClock.elapsedRealtime()-s);}
        @Override protected void dispatchDraw(Canvas canvas){long s=SystemClock.elapsedRealtime();super.dispatchDraw(canvas);recordDraw700(SystemClock.elapsedRealtime()-s);}
