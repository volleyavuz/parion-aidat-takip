...
/** v4.1.15 HOME single setContentView + suppress repeated layout during legacy callbacks. */
public class MainActivityV700 extends MainActivityV699 {
    ...
    static final class FastHomeRoot700 extends LinearLayout {
        ...
        private long slot700=0L; private int seq700=0; private int pending700=0;
        FastHomeRoot700(Context c){super(c);}

        private void beginMutation700(){ if(pending700++==0) suppressLayout(true); }
        private void endMutation700(){ if(--pending700<=0){ pending700=0; suppressLayout(false); requestLayout(); invalidate(); } }
        ...
        @Override public boolean postDelayed(Runnable action,long delayMillis){
            ...
            Runnable measured=()->{
                long started=SystemClock.elapsedRealtime(); beginMutation700();
                try { action.run(); }
                finally { endMutation700(); long cost=SystemClock.elapsedRealtime()-started; long queue=started-scheduled; recordCallback700(cost,queue,requested,seq); Log.i(TAG,"RUN #"+seq+" req="+requested+"ms actual="+actual+"ms queue="+queue+"ms cost="+cost+"ms class="+action.getClass().getName()); }
            };
